...
Kinematics::rpm Kinematics::calculateRPM(float linear_x, float linear_y, float angular_z)
{
    ...
    // ACKERMANN uses the sum of half front/rear wheelbase and half track width
    // as the effective turning radius reference, rather than half the track width alone.
...
Kinematics::rpm Kinematics::getRPM(float linear_x, float linear_y, float angular_z)
{
    ...
    {
        // ACKERMANN steers with a dedicated steering actuator (see steering.h/.cpp),
        // not by differential wheel speed. Only the drive motor (motor1) RPM matters.
...
// ACKERMANN-only overload: derives linear_x from the single drive motor's RPM and
// angular_z from the current commanded steering angle (bicycle model).
