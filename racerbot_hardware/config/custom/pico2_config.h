...
#define LINO_BASE ACKERMANN                // Ackermann (car-like) steering robot, ported from jimdinunzio/linorobot2_hardware
...
#define USE_ESC_MOTOR_DRIVER            // Drive motor ESC + steering servo (required for ACKERMANN)
...
#define FR_WHEELS_DISTANCE 0.200            // distance between front and rear wheels (ACKERMANN only) - MEASURE AND SET FOR YOUR CHASSIS
#define PWM_BITS 10                         // PWM Resolution of the microcontroller
#define PWM_FREQUENCY 20000                 // PWM Frequency
#define SERVO_BITS 12                       // Servo PWM resolution
#define SERVO_FREQ 100                      // Servo PWM frequency

// Steering geometry / servo travel limits (ACKERMANN only) - TUNE THESE to your servo and linkage
#define STEERING_FULL_RANGE_DEG   80         // total mechanical steering range in degrees
#define STEERING_HALF_RANGE_DEG   (STEERING_FULL_RANGE_DEG/2)
#define STEERING_HALF_RANGE_RAD   (STEERING_HALF_RANGE_DEG * PI / 180)
#define STEERING_MIN_TURN_RADIUS  (FR_WHEELS_DISTANCE / tan(STEERING_HALF_RANGE_DEG * PI / 180))
#define STEERING_FULL_RIGHT_PWM   460        // microsecond offset from neutral (1500us) - tune per servo
#define STEERING_FULL_LEFT_PWM    -400       // microsecond offset from neutral (1500us) - tune per servo
...
#define MOTOR_STR_INV false                 // invert if steering goes the wrong way
...
// MOTOR PINS
  ...
  #define MOTOR1_PWM 12                      // drive motor ESC signal pin
  #define MOTOR1_IN_A -1 //DON'T TOUCH THIS! This is just a placeholder
  #define MOTOR1_IN_B -1 //DON'T TOUCH THIS! This is just a placeholder

  // MOTOR2/3/4 kept defined but unused by ACKERMANN (single drive wheel).
  #define MOTOR2_PWM -1
  ...
  // Dedicated steering servo/ESC channel (ACKERMANN only)
  #define MOTOR_STR_PWM 13                   // steering servo signal pin (reused from MOTOR2 slot)
...
// RC receiver passthrough (Traxxas_RemCtl) - optional, for manual/RC drive +
// hardware e-stop. GP8/GP9 chosen because they're free on this board's existing
// pin map (SDA_PIN/SCL_PIN=4/5, encoders use 2,3,6,7,10,11,20,21, ESC uses 12,13).
// Both pins must support interrupts (all RP2040/RP2350 GPIOs do).
#define DRIVER_CONTROL                       // enables the /driver_control_mode topic + RC manual mode
#define THROTTLE_IN_ESTOP_PIN 8               // RC receiver throttle/estop channel
#define STEERING_IN_PIN 9                     // RC receiver steering channel
#define ESTOP_PWM_THRESHOLD 1700              // throttle channel pulse width (us) at/below which e-stop triggers
